...
fun AppRoot(viewModel: AppViewModel) {
    ...
    LaunchedEffect(uiState.user?.id) {
        // Polling keeps mobile data fresh while websocket STOMP integration is still pending.
        while (true) {
            delay(15_000)
    ...
    Scaffold(
        ...
        when (selectedTab) {
            MainTab.DASHBOARD -> DashboardScreen(uiState = uiState, onRefresh = viewModel::refreshAll, modifier = Modifier.padding(padding))
...
private fun LoginPanel(viewModel: AppViewModel, isLoading: Boolean) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.padding(top = 16.dp)) {
        ...
        TextButton(onClick = { viewModel.switchAuthMode(AuthMode.RECOVER) }, enabled = !isLoading) {
            Text("Forgot password?")
        }
        TextButton(onClick = { viewModel.switchAuthMode(AuthMode.REGISTER) }, enabled = !isLoading) {
...
private fun RegisterPanel(viewModel: AppViewModel, isLoading: Boolean) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.padding(top = 16.dp)) {
        ...
        Button(onClick = { viewModel.register(name, email, password) }, enabled = !isLoading) {
            Text(if (isLoading) "Creating account..." else "Create Account")
        }
        TextButton(onClick = { viewModel.switchAuthMode(AuthMode.LOGIN) }, enabled = !isLoading) {
...
private fun RecoverPanel(viewModel: AppViewModel, isLoading: Boolean) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.padding(top = 16.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(onClick = { viewModel.forgotPassword(email) }, enabled = !isLoading) {
        ...
        Button(onClick = { viewModel.resetPassword(token, newPassword) }, enabled = !isLoading) {
            Text("Reset Password")
        }
        TextButton(onClick = { viewModel.switchAuthMode(AuthMode.LOGIN) }, enabled = !isLoading) {
...
@Composable
private fun DashboardScreen(uiState: AppUiState, onRefresh: () -> Unit, modifier: Modifier = Modifier) {
